...
std::string to_string(ContactMethod contact_method) {
	if (contact_method == ContactMethod::SlaveNodes)
		return "slave-nodes";
	if (contact_method == ContactMethod::UniformLambdaPartition)
		return "uniform-lambda";
	return "uniform-union";
}

std::string to_string(ContactSlaveBody slave_body) {
	return slave_body == ContactSlaveBody::Bottom ? "bottom" : "top";
}
...
FSEM::FSEM(double E, double nu, const Point& a, const Point& b,
	...
	const int n_nodes = static_cast<int>(2 * (n_side_x + n_side_y));
...
void FSEM::set_bc2(const std::vector<size_t>& pos,
	...
	for (size_t i = 0; i < fem.psize(); i++) {
		...
		for (size_t j = 0; j < 4; ++j) {
			if (j == 0)
				...
				prev_pos = static_cast<int>(j - 1);

			if (j == 0 || j == 2)
				finish = static_cast<int>(2 * n_side_y);
			else
				finish = static_cast<int>(2 * n_side_x);

			calculate_coef_Matrix_bc2(
				finish,
				static_cast<int>(i),
				pos[j],
				pos[prev_pos],
	...
	if (!pos[0]) {
		...
		if (pos[1]) 
			save_bc1(coefs_Dirichle, dir_id, static_cast<int>(n_side_y));
	}

	if (!pos[1]) {
		
		for (int i = static_cast<int>(n_side_y);
			i < static_cast<int>(n_side_y + n_side_x); ++i)
			save_bc1(coefs_Dirichle, dir_id, i);

		if (pos[2]) 
			save_bc1(coefs_Dirichle, dir_id, static_cast<int>(n_side_y + n_side_x));
	}
	
	if (!pos[2]) {
		for (int i = static_cast<int>(n_side_x + n_side_y);
			i < static_cast<int>(n_side_x + 2 * n_side_y); ++i)
			save_bc1(coefs_Dirichle, dir_id, i);

		if (pos[3])
			save_bc1(coefs_Dirichle, dir_id, static_cast<int>(n_side_x + 2 * n_side_y));
	}

	if (!pos[3]) 
		for (int i = static_cast<int>(n_side_x + 2 * n_side_y);
			i < static_cast<int>(2 * (n_side_x + n_side_y)); ++i)
			save_bc1(coefs_Dirichle, dir_id, i);
	...
	void assemble_body_mortar_matrix(
		...
		for (const auto& mortar_element : mortar_elements) {
			...
			for (size_t node : side_nodes) {
				...
				const double weight = (coordinate_system == CoordinateSystem::Axisymmetric)
					? kAxisymmetricTwoPi * r_mid * len
					: len;
...
std::vector<double> solve_mortar_contact(
	...
	if (contact_left > contact_right + kContactEps)
		throw std::runtime_error("Bodies do not overlap along the contact boundary.");
	...
	const std::vector<double>& lambda_nodes = discretization.lambda_nodes;
	const std::vector<MortarElement>& mortar_elements = discretization.mortar_elements;
	...
	assemble_body_mortar_matrix(M1, basis_bottom, side_bottom, fem_bottom,
		bottom_x, mortar_elements, lambda_nodes, options.coordinate_system);
	assemble_body_mortar_matrix(M2, basis_top, side_top, fem_top,
		top_x, mortar_elements, lambda_nodes, options.coordinate_system);
...
std::vector<double> solve_mortar_contact(
	...
	options.coordinate_system = bottom_body.get_coordinate_system();
	options.method = ContactMethod::UniformUnionPartition;
	options.lambda_node_count = lambda_node_count;
	return solve_mortar_contact(bottom_body, top_body, rhs_bottom, rhs_top, options);
}
